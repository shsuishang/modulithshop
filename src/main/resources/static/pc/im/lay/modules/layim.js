  ...
  var elemChatTpl = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">'
    ...
    ,'<div class="layim-chat-footer">'
      ,'<div class="layui-unselect layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">'
        ...
        ,'<span class="layui-icon layim-tool-image" title="' + __('上传图片') + '" layim-event="image">&#xe60d;<input type="file" name="upfile" class="layim-image-input"></span>'
        ,'{{# }; }}'
        ...
        ,'{{# }; }}'
    , '{{# if(d.base && d.base.chatOrder){ }}'
    , '<span class="layim-tool-order" layim-event="chatOrder"><i class="layui-icon">&#xe60a;</i>' + __('我的订单') + '</span>'
    , '{{# }; }}'
    , '{{# if(d.base && d.base.chatBrowser){ }}'
    , '<span class="layim-tool-browser" layim-event="chatBrowser"><i class="layui-icon">&#xe630;</i>' + __('最近浏览') + '</span>'
    , '{{# }; }}'
      ,'</div>'
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,chatLog: function(othis){
      ...
      return events.chatLog.index = layer.open({
        ...
        , content: cache.base.chatLog + '?id=' + (thatChat.data.id || thatChat.data.user_id) + '&type=' + thatChat.data.type + '&store_id=' + thatChat.data.store_id + '&friend_id=' + thatChat.data.friend_id	 + '&api_url=' + encodeURIComponent(api_url)
      });
    }


    //我的订单
    , chatOrder: function (othis) {
      var thatChat = thisChat();
      if (!cache.base.chatOrder) {
        return layer.msg(__('尚无任何订单'));
      }
      layer.close(events.chatOrder.index);
      return events.chatOrder.index = layer.open({
        type: 2
        , maxmin: true
        , title: __('我的订单')
        , area: ['450px', '100%']
        ...
        , skin: 'layui-box'
        , anim: 2
        , id: 'layui-layim-chatorder'
        , content: cache.base.chatOrder + '?id=' + (thatChat.data.id || thatChat.data.user_id) + '&type=' + thatChat.data.type + '&store_id=' + thatChat.data.store_id + '&friend_id=' + thatChat.data.friend_id
      });
    }


    //浏览记录
    , chatBrowser: function (othis) {
      var thatChat = thisChat();
      if (!cache.base.chatBrowser) {
        return layer.msg(__('尚无最新浏览记录'));
      }
      layer.close(events.chatBrowser.index);
      return events.chatBrowser.index = layer.open({
        type: 2
        , maxmin: true
        , title: __('最近浏览')
        , area: ['450px', '100%']
        ...
        , skin: 'layui-box'
        , anim: 2
        , id: 'layui-layim-chatbrowser'
        , content: cache.base.chatBrowser + '?id=' + (thatChat.data.id || thatChat.data.user_id) + '&type=' + thatChat.data.type + '&store_id=' + thatChat.data.store_id + '&friend_id=' + thatChat.data.friend_id
      });
    }
